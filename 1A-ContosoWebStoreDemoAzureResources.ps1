...
##########################################################################################################################################################################
        ...
        do {
            ...
            if ($subscriptionID -notmatch"-") {Write-Host -ForegroundColor Magenta "`t-> Please enter a valid Azure Subscription ID." }
        }
        until (
        ...
        do {
            ...
            if ($sqlTDAlertEmailAddress -notmatch "@") {Write-Host -ForegroundColor Magenta "`t-> Please enter a valid email address."}
        }
        until (
...
Write-Host -ForegroundColor Green "`n###############################         Deploying to Azure         ###############################"
        ...
        try {
            ...
            Write-Host -ForegroundColor Magenta "Deployment failed at a templated step. Review the error in the Azure portal. Before redeploying, remove any previously created assets and attempt the deployment again."
            Break
        }
        ...
        try {
            ...
            Write-Host "`t* Getting access token and setting variables to invoke the REST API." -ForegroundColor Yellow
            ...
            Write-Host "`t* Creating an authorization header." -ForegroundColor Yellow
            ...
            Write-Host "`t* Retrieving data for $PolicyName policy..." -ForegroundColor Yellow
...
Write-Host -ForegroundColor Green "`n###############################         Deployment Running         ###############################"
